$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
			...
			{label: '回复ID', name: 'replyid', index: 'replyId', width: 80},
...
let vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
            let url = vm.wxMsg.id == null ? "../wxmsg/save" : "../wxmsg/update";
			$.ajax({
				type: "POST",
			    url: url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.wxMsg),
                success: function (r) {
                    if (r.code === 0) {
