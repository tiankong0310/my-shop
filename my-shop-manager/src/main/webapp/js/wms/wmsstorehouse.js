$(function () {
    $("#jqGrid").jqGrid({
        url: '../wmsstorehouse/list',
        datatype: "json",
        colModel: [
			{label: 'id', name: 'id', index: 'id', key: true, hidden: true},
			{label: '仓库编号', name: 'storehouseId', index: 'storehouse_id', width: 80},
			{label: '地址', name: 'address', index: 'address', width: 80},
			{label: '电话', name: 'phone', index: 'phone', width: 80},
			{label: '操作者', name: 'userId', index: 'user_id', width: 80},
...
let vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
            let url = vm.wmsStorehouse.id == null ? "../wmsstorehouse/save" : "../wmsstorehouse/update";
			$.ajax({
				type: "POST",
			    url: url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.wmsStorehouse),
                success: function (r) {
                    if (r.code === 0) {
		...
		getInfo: function(id){
			$.get("../wmsstorehouse/info/"+id, function (r) {
                vm.wmsStorehouse = r.wmsStorehouse;
            });
		},
